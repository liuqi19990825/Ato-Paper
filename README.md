...
一款为 Typecho 1.3.0 制作的纸张风个人博客主题，由 Ato 与 Codex 共同构建。当前版本为 0.10.5。
...
该功能默认开启，可在设置外观中随时关闭；转载协议可通过“复制出处协议”修改。默认仅在复制内容达到 80 个可见字符时追加声明，短句会保持普通复制；“复制声明触发字数”可以修改阈值，填写 `0` 表示任何长度都追加。计数时空格、换行和零宽字符不会占用字数，中文按一个字符计算。
...
主题附带 `manifest.json`、192px/512px 安装图标与 maskable 图标。Chrome 将博客安装到桌面后会以 `standalone` 模式打开，并使用纸张色填充状态栏、启动背景与设备安全区域；手动切换深色模式时，页面也会同步更新系统主题色。
