...
一款为 Typecho 1.3.0 制作的纸张风个人博客主题，由 Ato 与 Codex 共同构建。当前正式版本为 1.0.3。

## 项目说明

Ato Paper 是一个通过 **vibe coding** 完成的个人主题项目：Ato 负责需求、审美取向与实际使用反馈，Codex 协助设计、实现、检查和迭代。代码公开是为了分享主题、保留版本记录，也方便感兴趣的人继续探索这种纸张风博客体验。

本仓库采用个人维护方式，**不接受 Pull Request 或其他形式的代码合并**。如果你想调整功能、尝试新的设计或发展自己的版本，请直接 Fork 本仓库并在自己的分支上自由修改；使用和再发布时请遵守 MIT License 以及 `THIRD_PARTY_NOTICES.md` 中列出的第三方许可证。

## 当前功能

- 纸张质感首页与阅读型文章页
- 桌面端首页、文章与独立页面共用 920px 外层框架；无目录内容在 720px 阅读轨道内居中，开启目录时正文左置并为右侧导航留出空间
- 主题根目录附带经过压缩的首页预览图，Typecho 外观管理页可直接显示主题截图
- 自托管 Noto Serif SC 与 Noto Sans SC 可变字体，统一 Windows、macOS、Android 与 iOS 的主要排版
- 深色模式与本地偏好记忆
- 分类、标签、作者和搜索归档
- Typecho 原生评论与嵌套回复
- 独立“碎碎念”页面：从指定的 Typecho 文章分类读取内容并分页展示
- 碎碎念文章未填写标题时自动隐藏 Typecho 的“未命名文档”占位标题
- 指定的碎碎念分类会从首页普通文章流中排除，首页分页数量会同步重算
- 首页“随便逛逛”会同步隐藏碎碎念分类，侧栏 RSS 后提供快捷后台入口
- 后台可配置首页文案、插图、社交链接、备案信息和碎碎念分类
- ICP 与公安备案号分别按填写状态显示，未填写的项目不会输出占位文字
- 响应式手机布局、键盘焦点与减少动效支持
- 手机菜单使用紧凑面板，子页面通过缩进与小号 L 形连接角表达层级，并以较深底色标记当前页面
- 提供 Web App Manifest、安装图标、独立显示模式及移动设备安全区域适配
- 文章副标题、题图和题图说明自定义字段
...
- 文章与独立页面的 Markdown 图片自动组成灯箱相册，支持触摸、键盘、缩放、图注及缩略图链接原图
- 可选的正文复制出处功能，在剪贴板末尾追加作者、原文链接、来源和可配置转载协议
- 纸张风 404 页面提供站内搜索、返回首页与最近文章入口，并保持正确的 HTTP 404 状态
- 普通文章的上一篇/下一篇导航会跳过碎碎念分类，避免两种内容流互相穿插
- 桌面端主滚动条使用与纸张配色一致的窄轨道和圆角滑块，深色模式同步适配

## 1.0.0 正式版

1.0.0 在 0.10.5 的既有结构上完成稳定性审计，没有进行大面积重构。正式版统一了主题元数据与默认配置，对主题设置中的外部链接执行 HTTP(S) 校验，为超长代码块限制高开销的自动语言识别，并复用同一请求内的碎碎念分类关系查询。

默认首页插图改用经过视觉检查的 WebP 资源，安装图标与 favicon 也进行了无明显视觉差异的压缩；自托管中文字体仍保留官方 `unicode-range` 分片，以维持跨平台字体一致性和按需加载能力。完整版本记录见 `CHANGELOG.md`。
...
碎碎念使用 Typecho 原生文章发布，不再需要每次进入主题设置编辑文本：

1. 在“管理 → 分类”中新建一个分类，例如名称填写“碎碎念”，缩略名填写 `murmurs`。
2. 在后台新建或编辑独立页面，标题填写“碎碎念”，缩略名可继续使用原来的 `now`；展开高级选项，将自定义模板选为“碎碎念”。已有的近况页面不必重建，只需改标题并保留 `page-now.php` 模板。
3. 进入“控制台 → 外观 → 设置外观”，在“碎碎念分类”中选择第 1 步创建的分类，并确认“碎碎念页面地址”指向第 2 步的页面。默认地址仍是 `now.html`。
4. 以后像写普通文章一样新建文章，放入“碎碎念”分类后发布。正文可以使用 Markdown、图片和附件，每一条也保留独立文章地址与原生评论。

启用后，属于该分类的文章不会重复出现在首页“最近写的”文章流，首页分页也会按排除后的文章数量重新计算；首页“最近在做”便签会自动摘取最新一条碎碎念，点击后进入碎碎念独立页。分类归档页仍可正常访问。

“碎碎念每页条数”默认为 8。若暂时不选择分类，主题会继续读取“旧版碎碎念数据（兼容）”中的 `日期|标签|标题|正文` 文本，因此升级不会立刻丢失原有展示；完成迁移后可以保留旧数据作备份，也可以清空。
...
在“控制台 → 外观 → 设置外观”中将“复制正文时追加出处”设为开启后，从文章或普通独立页面正文复制文字，会在剪贴板末尾自动附加：
...
该功能默认关闭，可在设置外观中手动开启；转载协议可通过“复制出处协议”修改。开启后，默认仅在复制内容达到 80 个可见字符时追加声明，短句会保持普通复制；“复制声明触发字数”可以修改阈值，填写 `0` 表示任何长度都追加。计数时空格、换行和零宽字符不会占用字数，中文按一个字符计算。

它同时写入剪贴板的纯文本与富文本格式，但不会处理评论、导航和其他界面文字，也不会影响代码块的选中复制或一键复制。功能交互灵感来自 Sakura，剪贴板逻辑为 Ato Paper 独立实现。

## 图片灯箱与文章相册

文章和普通独立页面中的 Markdown 图片会自动获得看大图功能，同一页面里的图片会组成一组，可通过左右按钮、键盘方向键或手机滑动连续浏览。图片的 `alt` 文本会作为灯箱图注：

```markdown
![窗边的下午](https://example.com/photo.jpg)
```

如果原图较大，可以使用标准 Markdown 的“缩略图链接原图”写法。页面先加载较小的缩略图，点击后再读取原图：

```markdown
[![窗边的下午](https://example.com/photo-thumb.jpg)](https://example.com/photo-original.jpg)
```

链接到普通网页的图片不会被灯箱接管，仍会正常跳转。如果某张图片不需要看大图，可使用 HTML 并添加 `data-no-lightbox`：

```html
<img src="https://example.com/banner.jpg" alt="横幅" data-no-lightbox>
```

灯箱资源随主题本地提供，不请求外部 CDN，并会在 PJAX 页面切换后自动重新初始化。
...
主题附带 `manifest.json`、192px/512px 安装图标，512px 图标同时声明为 maskable。Chrome 将博客安装到桌面后会以 `standalone` 模式打开，并使用纸张色填充状态栏、启动背景与设备安全区域；手动切换深色模式时，页面也会同步更新系统主题色。
